...
class SDT(nn.Module):
    ...
    def __init__(
        ...
        self.inner_nodes = nn.Sequential(
            nn.Linear(self.output_dim +1, self.internal_node_num_, bias=False),
            nn.Sigmoid(),
        )
    ...
    def _forward(self, X):
        ...
        # Data augmentation: add a constant '1' to each sample
        X = self._data_augment(X)

        # path_prob has shape: [batch_size, #internal_nodes, 2],
        # but it's manipulated layer by layer below
