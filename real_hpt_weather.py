...
window_sizes = [6, 18, 36]
shifts = [1, 6, 144, 288]
lags = [1, 2, 3, 4, 5, 6, 12, 18, 36, 144, 288]


# Results of hyperparameter search for future reference
best_params_lgb = {'num_leaves': 31, 'n_estimators': 500, 'max_depth': -1, 'learning_rate': 0.05}
best_params_bsdt = {'boosting_iters': 1, 'criterion': 'mse', 'depth': 4, 'epochs': 20, 'lr_sdt': 0.1}
best_params_bsdtq = {'boosting_iters': 1, 'criterion': 'mse', 'depth': 3, 'epochs': 20, 'how_many_grad': 50, 'iterate_until_converge': 20, 'lr_q': 0.1, 'lr_sdt': 0.1, 'output_dim': 5}
...
train = df.drop(index=test.index).groupby(
    "unique_id", group_keys=False).sample(n=N, random_state=seed)
...
if hpt_lgb:
    ...
    model_lgb = LGBMRegressor(verbosity=-1, **best_params_lgb)
